...
import "com.androlua.*"
import "vinx.abverterx.ripplelua.core.*"
...
function sort(a,b)
  return string.lower(a.appname or "") < string.lower(b.appname or "")
end
...
for k,v in ipairs(pls) do
  ...
  adp.add{icon=checkicon(i),title=(v.appname or  "").." "..v.appver,path=v.path,description=v.description or ""}
end
_adp=adp
plist.Adapter=adp
