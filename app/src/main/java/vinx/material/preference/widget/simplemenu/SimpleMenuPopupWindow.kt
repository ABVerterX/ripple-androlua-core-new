...
open class SimpleMenuPopupWindow @SuppressLint("InflateParams") constructor(
    ...
    private val margin = Array(2) { IntArray(2) }
    val listPadding = Array(2) { IntArray(2) }
    private val itemHeight: Int
    private val dialogMaxWidth: Int
    ...
    constructor(
        ...
        context, attrs, defStyleAttr, R.style.Widget_Preference_SimpleMenuPreference_PopupMenu
    )
    ...
    init {
        ...
        listPadding[DIALOG][VERTICAL] = (context.resources.displayMetrics.density * 8).roundToInt()
...
//            }
    ...
    private fun showDialog(parent: View, container: View) {
        ...
        width = dialogMaxWidth.coerceAtMost(container.width - margin[DIALOG][HORIZONTAL] * 2)
    ...
    private fun showPopupMenu(anchor: View, container: View, width: Int, extraMargin: Int) {
        ...
        if (measuredHeight > containerHeight) {
            ...
            val scroll =
                (itemHeight * index - anchorTop) + listPadding[POPUP_MENU][VERTICAL] + margin[POPUP_MENU][VERTICAL]
        ...
        val animElevation = (elevation * 0.25).roundToInt().toInt()
        contentView.post {
            SimpleMenuAnimation.startEnterAnimation(
                background,
                contentView,
    ...
    private fun measureWidth(maxWidth: Int, entries: Array<CharSequence>): Int {
        ...
        Arrays.sort(
            entries
        ) { o1: CharSequence, o2: CharSequence -> o2.length - o1.length }
