...
public class LuaDexLoader {
    private static final HashMap<String, LuaDexClassLoader> dexCache = new HashMap<>();
    ...
    public DexClassLoader loadDex(String path) throws LuaException {
        ...
        if (dex == null) {
            ...
            if (!new File(path).exists()) if (new File(path + ".dex").exists()) path += ".dex";
            else if (new File(path + ".jar").exists()) path += ".jar";
            else throw new LuaException(path + " not found");
    ...
    public void loadResources(String path) {
        try {
            ...
            mResources = new LuaResources(mAssetManager, superRes.getDisplayMetrics(), superRes.getConfiguration());
